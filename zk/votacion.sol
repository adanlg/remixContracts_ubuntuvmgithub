...
contract Tornado is ReentrancyGuard {
    ...
    uint public nyb = 0;
    uint public vips = 0;
    uint public tierra = 0;
    uint public chino = 0;
    uint public sodexo = 0;
    uint public grosso = 0;
    uint8 public treeLevel = 10;
    //uint256 choosenRestaurant;
...
function canWithdraw(
    ...
    uint[2] memory c,
    uint[2] memory input,
    uint256 _choosenRestaurant
) public returns (bool) {
    uint256 _root = input[0];
    uint256 _nullifierHash = input[1];
    uint256 _addr = uint256(uint160(owner));
    require(_choosenRestaurant >= 1 && _choosenRestaurant <= 6, "Invalid restaurant choice");
    // Check if root is valid
    
    require(!nullifierHashes[_nullifierHash], "already-spent");
    require(roots[_root], "Not a valid root");

    bool verifyOK = verifier.verifyProof(a, b, c, [_root, _nullifierHash, _addr]);
    require(verifyOK == true, "not valid proof");

    if (_choosenRestaurant >= 1 && _choosenRestaurant <= 6) {
        if (_choosenRestaurant == 1) nyb += 1;
        else if (_choosenRestaurant == 2) vips += 1;
        else if (_choosenRestaurant == 3) tierra += 1;
        else if (_choosenRestaurant == 4) chino += 1;
        else if (_choosenRestaurant == 5) sodexo += 1;
        else if (_choosenRestaurant == 6) grosso += 1;
        // Record the vote or action for the chosen restaurant
        return true; // Indicate success
    } else {
        return false; // Indicate failure or invalid choice
    }
    }
}